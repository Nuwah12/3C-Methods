...
#Fetch genomic regions and load data
chromsizes = bioframe.fetch_chromsizes(args.genome) #Chromosome sizes
chromosomes = list(chromsizes.index) #Chromosome names
hg38_cens = bioframe.fetch_centromeres(args.genome) #Centromeres
hg38_arms = bioframe.make_chromarms(chromsizes, hg38_cens) #Chromosome arms
...
if args.filetype == 'bed':
    ...
    paired_sites.to_csv("CTCF_bioframe_paired_sites.bed", sep="\t", index=False)

print("{}: Pileup and average snips . . .".format(datetime.datetime.now()))
#Make snips:
stack = cooltools.pileup(cooler_obj, paired_sites, view_df=hg38_arms, expected_df=expected, flank=args.map_flank)
#Aggregate snips:
